...
const BookAppointmentPage: React.FC = () => {
  ...
  useEffect(() => {
    if (formData.clinicId) {
      const loadDoctors = async () => {
        try {
          const response = await getDoctors(formData.clinicId, formData.serviceId || undefined);
          if (response.isSuccess && response.data) {
  ...
  }, [formData.clinicId, formData.doctorId, formData.selectedDate, formData.serviceId]);
  ...
  const handleSubmitBooking = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.isSuccess) {
        ...
        // Navigate to appointments page if authenticated, otherwise to home
        setTimeout(() => {
          if (isAuthenticated) {
...
const HeaderStepper: React.FC<{ step: Step; isAuthenticated: boolean }> = ({ step, isAuthenticated }) => {
  return (
          ...
          >
            ...
            {services.map((service) => (
              <option key={service.serviceId} value={service.serviceId}>
                {service.name} - {service.defaultPrice?.toLocaleString()}đ
              </option>
            ))}
          </select>
        </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
              {slots.map((slot, index) => {
                const isSelected = data.selectedSlot?.startAt === slot.startAt;
                return (
                  <button
                    key={index}
                    type="button"
...
const StepConfirmation: React.FC<StepConfirmationProps> = ({
  ...
  const selectedService = services.find(s => s.serviceId === data.serviceId);
    ...
    <>
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div className="bg-slate-50 rounded-lg p-4 mt-4">
          <h3 className="text-xs font-semibold text-slate-700 mb-3">Tóm tắt thông tin:</h3>
          <div className="space-y-2 text-xs text-slate-600">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-right">{selectedClinic?.name}</span>
            </div>
            {selectedService && (
              <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span>{data.selectedSlot ? formatDate(data.selectedSlot.startAt) : "-"}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Giờ:</span>
              <span>
                {data.selectedSlot
                  ? `${formatTime(data.selectedSlot.startAt)} - ${formatTime(data.selectedSlot.endAt)}`
                  : "-"}
              </span>
            </div>
            {isAuthenticated && (
              <>
                ...
                <div className="flex items-center gap-2 text-blue-600">
                  ...
                  <span className="text-xs">Lịch hẹn sẽ được lưu vào hồ sơ của bạn</span>
                </div>
              </>
            )}
          </div>
