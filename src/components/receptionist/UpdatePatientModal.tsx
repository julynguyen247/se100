...
import { updatePatient, getClinics, type CreatePatientRequest, type ClinicDto, type PatientDetail } from "@/services/apiReceptionist";
...
const UpdatePatientModal: React.FC<UpdatePatientModalProps> = ({ isOpen, onClose, onSuccess, patient }) => {
    ...
    useEffect(() => {
        if (patient && isOpen) {
            ...
            setFormData({
                clinicId: clinics[0]?.id || "", // Will be updated when clinics load
                patientCode: "", // Backend doesn't return patient code
                fullName: patient.name,
                gender: "Male", // Backend doesn't return gender, default to Male
    ...
    const fetchClinics = async () => {
        try {
            ...
            if (result.isSuccess && result.data) {
                ...
                if (result.data.length > 0 && !formData.clinicId) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.isSuccess) {
                ...
                toast.error(result.message || 'Không thể cập nhật bệnh nhân');
            }
        } catch (err: any) {
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-200 sticky top-0 bg-white rounded-t-2xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-semibold text-slate-900">Cập nhật thông tin bệnh nhân</h2>
                            <p className="text-xs text-slate-500">Chỉnh sửa thông tin bệnh nhân</p>
                        </div>
                    </div>
                    <button
                        onClick={handleClose}
