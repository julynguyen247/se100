# 🎯 PROMPT: Xử Lý Patient Dashboard với Real API Data

## 📋 Yêu Cầu

Xây dựng Patient Dashboard component với các tính năng:
- Hiển thị tên bệnh nhân từ API
- 3 stats cards: Lịch hẹn sắp tới, Điều trị hoàn thành, Lần khám gần nhất
- Danh sách lịch hẹn sắp tới (2 items)
- Lịch sử điều trị (2-3 items)
- Loading states và error handling

---

## 🔌 API Endpoints Cần Dùng (Theo Swagger Thực Tế)

### ⚠️ LƯU Ý QUAN TRỌNG
- **KHÔNG có** `GET /api/auth/me` trong Swagger
- **KHÔNG có** `GET /api/patients/{patientId}/dashboard-summary` trong Swagger  
- **KHÔNG có** `GET /api/treatments` trong Swagger
- Tất cả API `/api/patient/*` tự động lấy patientId từ JWT token (không cần truyền patientId)
